...
render() {
    ...
    if(this.state.userDetail.length>0){
        ...
        arr1.map((data1,index) => {
          var dateAt = new Date(data1.date);
            if(data1.all.declined && (dateAt >= this.state.startDate) && (dateAt <= this.state.toDate)){
    ...
    data2arr.map((dataCsv,index) => {
      ...
      if(dataCsv.all.declined === true && (dateAt2 >= this.state.startDate) && (dateAt2 <= this.state.toDate)){
    ...
    let filename = "Declined_users_list_"+dateFormat+".csv";
  return (
