...
render() {
    ...
    if(this.state.userDetail.length>0){
        ...
        arr1.map((data1,index) => {
          var dateAt = new Date(data1.date);
            if(!data1.all.declined && data1.action && (dateAt >= this.state.startDate) && (dateAt <= this.state.toDate) && data1.all.level !== 'silver' && data1.all.level !== 'gold' && data1.all.level !== 'emarald' ){
    ...
    data2arr.map((dataCsv,index) => {
      ...
      if(!dataCsv.all.declined && dataCsv.all.action && dataCsv.all.active && (dateAt2 >= this.state.startDate) && (dateAt2 <= this.state.toDate) && dataCsv.all.level !== 'silver' && dataCsv.all.level !== 'gold' && dataCsv.all.level !== 'emarald' ){
