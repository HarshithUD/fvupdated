...
render() {
    ...
    if(this.state.userDetail.length>0){
        ...
        arr1.map((data1,index) => {
          var dateAt = new Date(data1.date);
            if(!data1.all.declined && data1.action && (dateAt >= this.state.startDate) && (dateAt <= this.state.toDate) && data1.all.level !== 'silver' ){
            return resultData[index] = {
                slno:++count,
                name:data1.name,
                email:data1.email,
                stage:data1.all.level==='gold'?data1.all.stage:data1.all.level==='sapphire'?data1.all.stage-10:data1.all.stage,
                level:<span class="badge badge-secondary" style={{fontSize:'13px',fontWeight:'400'}}>{data1.all.level==='gold'?data1.all.level:'gold'}</span>,
                date:data1.date,
    ...
    data2arr.map((dataCsv,index) => {
      ...
      if(!dataCsv.all.declined && dataCsv.all.action && dataCsv.all.active && (dateAt2 >= this.state.startDate) && (dateAt2 <= this.state.toDate) && dataCsv.all.level !== 'silver' ){
      let rsname = dataCsv.all.name;
      let rsid = dataCsv.all._id;
      let rsdate = dataCsv.all.date;
      let rsstage = dataCsv.all.level==='gold'?dataCsv.all.stage:dataCsv.all.level==='sapphire'?dataCsv.all.stage-10:dataCsv.all.stage;
      let rslevel = dataCsv.all.level==='gold'?dataCsv.all.level:'gold';
